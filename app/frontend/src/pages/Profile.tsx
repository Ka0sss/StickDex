...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const loadCollections = async () => {
      ...
      setError(null)
      try {
        const data = await api<CollectionSummary[]>(`/collections?userId=${userId}`)
        if (active) setCollections(data)
      } catch (err: unknown) {
        if (active) {
          setError(err instanceof ApiError ? err.message : 'Error al cargar tus colecciones')
        }
      } finally {
      ...
      <div className="flex flex-col justify-between gap-6 border-b border-binder-800/80 pb-6 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm font-medium text-slate-400">
            Colecciones de <b className="text-white">{user?.username}</b>, tanto públicas como
            privadas.
          </p>
        </div>
        ...
        <Link
          to="/collections"
          className="inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-amber-400 via-amber-500 to-amber-600 px-5 py-2.5 text-xs font-black uppercase tracking-wider text-slate-950 shadow-lg shadow-amber-500/20 transition hover:brightness-110"
        >
      ...
      {loading ? (
        ...
        <div className="rounded-3xl border border-dashed border-binder-700/80 bg-binder-900/40 py-20 text-center">
          ...
          <p className="mt-1 text-xs text-slate-400">
            Inicia tu primera colección para empezar a registrar tus láminas y repetidas.
          </p>
          <Link
            to="/collections"
            className="mt-6 inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-amber-400 via-amber-500 to-amber-600 px-5 py-2.5 text-xs font-black uppercase tracking-wider text-slate-950 shadow-lg shadow-amber-500/20 transition hover:brightness-110"
          >
            Nueva Colección
          </Link>
        </div>
              ...
              >
                <div>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="font-mono text-xs font-black text-slate-400">#{col.id}</span>
                  </div>
